...
// to (with a final "wider world" group for the trio's own story and the lands
// they pass through). An adventure group's `link` resolves to its Compendium
// page; each card may also carry its own `link`. This is the trio's world, so
// the cold plot and characters live here in the Chronicles, not the Compendium.
const LORE_GROUPS = [
  {
  ...
  },
  {
    ...
    people: [
      ...
      { label: "Outskirt", name: "The Three Dwarves", link: "The Misty Island", description: "The three soaked, shaking eyewitnesses who burst into the Three Stags with the tale no one in Outskirt will repeat: the mist parting on the Grey, an island where none had been, a tall fire-licked shape on its tower. Their terror is the spark, then they flee east and don't look back." },
      ...
      { label: "The Misty Island", name: "The Sorcerer", link: "The Misty Island", description: "The dead master of the drowned hall, who reached through the thin skin of the world for something he couldn't hold and burned for it, days before the trio set out. He left the Hollow Mark cut in the rock. The module names him Sith." },
    ],
    places: [
    ...
    locations: [
      ...
      { label: "Outskirt", name: "Greatstead", link: "The Misty Island", description: "The largest farm in Outskirt: Warge and Luvia's loud, full, loving house, and their children Vagnhild and Biorn. The home none of the trio ever had, which is why they talk its children off the road." },
    ...
    items: [
      ...
      { label: "Map", name: "The Burned Map", link: "The Misty Island", description: "A half-burned but genuine map of the Misty Island, sold to the trio by the unreliable Sirter for fifteen gold. He took it off a traveller who died on the west track." },
...
function LoreGrid({ items, onOpenPage, groupTarget }) {
  return (
    <div className="lore-grid">
      {items.map((item) => {
        // A card whose subject has its own Compendium page links to it.
        const target = item.link ? resolvePage(item.link) : null;
        if (target && onOpenPage) {
          // The group header already links to the adventure, so a card that
          // points to that same page stays clickable but drops the repeated CTA.
          const sameAsGroup = groupTarget && groupTarget.kind === target.kind && groupTarget.id === target.id;
          return (
            <button
              key={item.name}
              className="lore-card lore-card--link"
    ...
    <section id="chronicles" className="media-section">
      ...
      <div className="chronicles-sections">
        ...
        {LORE_GROUPS.map((group) => {
          ...
          return (
            <div key={group.title} className="chronicles-sub">
              {groupTarget && onOpenPage ? (
                <button
                  className="chronicles-group__hd"
                  onClick={() => onOpenPage(groupTarget)}
                >
                  <h3 className="chronicles-sub__title chronicles-sub__title--bare">{group.title}</h3>
                  <span className="chronicles-group__cta">Read the adventure ↗</span>
                </button>
              ) : (
                <h3 className="chronicles-sub__title">{group.title}</h3>
              )}

              {LORE_TYPES.map(({ key, label }) =>
                group[key]?.length ? (
                  <div key={key} className="chronicles-type">
                    <h4 className="chronicles-type__title">{label}</h4>
