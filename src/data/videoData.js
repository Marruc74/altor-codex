...
// Markdown-only pages (no chronicle video) come from
// compendiumRegistry.generated.js, built by
// scripts/generate-compendium-registry.mjs from the files themselves (so adding
// a page no longer means hand-editing arrays here). Any registry page whose
...
const hiddenEntries = [];
{
  ...
  for (const section of touched) {
    const sec = videosBySection[section];
    sec.sort((a, b) => (a.group === null ? -1 : b.group === null ? 1 : a.group.localeCompare(b.group)));
